...
/**
 * <p>Objects of class "Bands" compute the conflict prevention 
 ...
 *
 * <p>If bands are set to conflict bands only, e.g, setConflictBands,
 ...
 * These methods have to be used before adding any traffic information.</p>
 *
 * <p>Note that in the case of geodetic coordinates, bands performs an 
 ...
 * 
 * <p>Disclaimers: Only parts of these algorithms have been formally
...
public class Bands implements GenericDHStateBands {                     
  ...
  public double getLookaheadTime(String unit) {
  ...
  public boolean solidVerticalSpeedBand(BandsRegion br) {
    if (verticalSpeedBandsLength() < 0) {
      return false;
    } else if (conflictBands && br == BandsRegion.NONE) {
      return verticalSpeedBandsLength() == 0;
    } else if (verticalSpeedBandsLength() > 0){
      return false;
    } else {
      return 
  ...
  private static int find_first_implicit_none(double tolerance,
    ...
    if (arraylist.get(0).low - lb > tolerance) {
    	return 0;
    }
    for (int i = 1; i < arraylist.size(); ++i) {
      if (arraylist.get(i).low -arraylist.get(i-1).up > tolerance) {
        return i;
      }
    }
    if (ub - arraylist.get(arraylist.size()-1).up > tolerance) {
