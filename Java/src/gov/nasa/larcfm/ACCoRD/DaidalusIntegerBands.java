...
public abstract class DaidalusIntegerBands {
	 ...
	 */ 
	public boolean no_CD_future_traj(Detection3D conflict_det, Optional<Detection3D> recovery_det, double B, double T, double B2, double T2, boolean trajdir, double tsk, 
			DaidalusParameters parameters, TrafficState ownship, TrafficState traffic, int target_step, boolean instantaneous) {
		return !CD_future_traj(conflict_det,B,T,trajdir,tsk,parameters,ownship,traffic,target_step,instantaneous) &&
				!(recovery_det.isPresent() && CD_future_traj(recovery_det.get(),B2,T2,trajdir,tsk,parameters,ownship,traffic,target_step,instantaneous));
	}
	...
	private boolean kinematic_red_band_exist(Detection3D conflict_det, Optional<Detection3D> recovery_det, double tstep, 
		...
		return (usehcrit && kinematic_first_nonrepulsive_step(tstep,trajdir,max,parameters,ownship,traffic,epsh) >= 0) ||
				...
				(recovery_det.isPresent() && kinematic_any_conflict_step(recovery_det.get(),tstep,B2,T2,trajdir,max,parameters,ownship,traffic));
	}
	...
	private boolean all_kinematic_red(Detection3D conflict_det, Optional<Detection3D> recovery_det, double tstep,
		...
		boolean leftans = dir > 0 || first_kinematic_green(conflict_det,recovery_det,tstep,B,T,B2,T2,false,maxl,parameters,ownship,traffic,epsh,epsv) < 0;
		boolean rightans = dir < 0 || first_kinematic_green(conflict_det,recovery_det,tstep,B,T,B2,T2,true,maxr,parameters,ownship,traffic,epsh,epsv) < 0;
	...
	private boolean any_kinematic_red(Detection3D conflict_det, Optional<Detection3D> recovery_det, double tstep,
			...
			int maxl, int maxr, DaidalusParameters parameters, TrafficState ownship, TrafficState traffic,
			int epsh, int epsv, int dir) {
		boolean leftred = dir <= 0 && kinematic_red_band_exist(conflict_det,recovery_det,tstep,B,T,B2,T2,false,maxl,parameters,ownship,traffic,epsh,epsv);
		boolean rightred = dir >= 0 && kinematic_red_band_exist(conflict_det,recovery_det,tstep,B,T,B2,T2,true,maxr,parameters,ownship,traffic,epsh,epsv);
