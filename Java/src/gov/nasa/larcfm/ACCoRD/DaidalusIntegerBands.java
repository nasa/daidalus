...
public abstract class DaidalusIntegerBands {
   ...
   LET kts = k*ts,
	...
	public boolean LOS_at(Detection3D det, boolean trajdir, double tsk, 
		...
		own.setPosition(new Position(sat));
	...
	private int first_los_step(Detection3D det, double ts,boolean trajdir,
			int min, int max, TrafficState ownship, TrafficState traffic) {
	...
	private int bands_search_index(Detection3D conflict_det, Optional<Detection3D> recovery_det, double tstep,
			...
			boolean trajdir, int max, TrafficState ownship, TrafficState traffic, 
			int epsh, int epsv) {
	...
	public boolean no_CD_future_traj(Detection3D conflict_det, Optional<Detection3D> recovery_det, double B, double T, double B2,
			double T2, boolean trajdir, double tsk, TrafficState ownship, TrafficState traffic) {
		return !CD_future_traj(conflict_det,B,T,trajdir,tsk,ownship,traffic) &&
				!(recovery_det.isPresent() && CD_future_traj(recovery_det.get(),B2,T2,trajdir,tsk,ownship,traffic));
	}
	...
	public void kinematic_bands_combine(List<Integerval> l, Detection3D conflict_det, Optional<Detection3D> recovery_det, double tstep,
			double B, double T, double B2, double T2,
			int maxl, int maxr, TrafficState ownship, TrafficState traffic, 
			int epsh, int epsv) {
	...
	public int first_green(Detection3D conflict_det, Optional<Detection3D> recovery_det, double tstep,
		...
		for (int k=0; k <= max; ++k) {
			...
			if ((tsk >= B && tsk <= T && LOS_at(conflict_det,trajdir,tsk,ownship,traffic)) ||
					...
					LOS_at(recovery_det.get(),trajdir,tsk,ownship,traffic)) ||
	...
	public boolean all_int_red(Detection3D conflict_det, Optional<Detection3D> recovery_det, double tstep,
		...
		boolean leftans = dir > 0 || first_green(conflict_det,recovery_det,tstep,B,T,B2,T2,false,maxl,ownship,traffic,epsh,epsv) < 0;
		boolean rightans = dir < 0 || first_green(conflict_det,recovery_det,tstep,B,T,B2,T2,true,maxr,ownship,traffic,epsh,epsv) < 0;
	...
	public boolean all_instantaneous_red(Detection3D conflict_det, Optional<Detection3D> recovery_det, 
			...
			int maxl, int maxr, TrafficState ownship, TrafficState traffic, 
			int epsh, int epsv, int dir) {
		boolean leftans = dir > 0 || first_instantaneous_green(conflict_det,recovery_det,B,T,B2,T2,false,maxl,ownship,traffic,epsh,epsv) < 0;
		boolean rightans = dir < 0 || first_instantaneous_green(conflict_det,recovery_det,B,T,B2,T2,true,maxr,ownship,traffic,epsh,epsv) < 0;
		return leftans && rightans; 
	}
	...
	public boolean any_int_red(Detection3D conflict_det, Optional<Detection3D> recovery_det, double tstep,
			...
			int maxl, int maxr, TrafficState ownship, TrafficState traffic,
			int epsh, int epsv, int dir) {
		boolean leftred = dir <= 0 && red_band_exist(conflict_det,recovery_det,tstep,B,T,B2,T2,false,maxl,ownship,traffic,epsh,epsv);
		boolean rightred = dir >= 0 && red_band_exist(conflict_det,recovery_det,tstep,B,T,B2,T2,true,maxr,ownship,traffic,epsh,epsv);
	...
	public boolean any_instantaneous_red(Detection3D conflict_det, Optional<Detection3D> recovery_det, 
			...
			int maxl, int maxr, TrafficState ownship, TrafficState traffic, 
			int epsh, int epsv, int dir) {
		boolean leftred = dir <= 0 && instantaneous_red_band_exist(conflict_det,recovery_det,B,T,B2,T2,false,maxl,ownship,traffic,epsh,epsv);
		boolean rightred = dir >= 0 && instantaneous_red_band_exist(conflict_det,recovery_det,B,T,B2,T2,true,maxr,ownship,traffic,epsh,epsv);
		return leftred || rightred; 
	}
	...
	public boolean no_instantaneous_conflict(Detection3D conflict_det, Optional<Detection3D> recovery_det, double B, double T, double B2, double T2,
			boolean trajdir, TrafficState ownship, TrafficState traffic,
			int epsh, int epsv) {
		boolean usehcrit = epsh != 0;
		boolean usevcrit = epsv != 0;    
