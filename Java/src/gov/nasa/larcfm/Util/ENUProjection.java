...
public final class ENUProjection implements EuclideanProjection {
	...
	public double conflictRange(double lat, double accuracy) {
		...
		double dist = Units.from("NM", Math.floor(243.0*Math.pow(Units.to("NM",Math.ceil(accuracy)),1.0/3.0))); // was 243.5, but this is slightly too high in some cases
//		if (dist < 7000) dist = 7000; // error should be < 0.01 m
		return dist;
