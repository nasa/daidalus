...
bool DaidalusAltBands::conflict_free_traj_step(const Detection3D* conflict_det, const Detection3D* recovery_det, double B, double T, double B2, double T2,
  ...
  if (instantaneous) {
    ...
    if ((tsqj2>=B &&
        CD_future_traj(conflict_det,B,std::min(T,tsqj2),trajdir,std::max(tsqj1,0.0),parameters,ownship,traffic,target_step,instantaneous)) ||
        (recovery_det != NULL && tsqj2>=B2 &&
            CD_future_traj(recovery_det,B2,std::min(T2,tsqj2),trajdir,std::max(tsqj1,0.0),parameters,ownship,traffic,target_step,instantaneous))) {
      return false;
    }
    ...
    return no_CD_future_traj(conflict_det,recovery_det,B,T,B2,T2,trajdir,std::max(tsqj3,0.0),parameters,ownship,traffic,target_step,instantaneous);
  }
}
...
void DaidalusAltBands::none_bands(IntervalSet& noneset, const Detection3D* conflict_det, const Detection3D* recovery_det,
  ...
  alt_bands_generic(altint,conflict_det,recovery_det,B,T,0.0,B,maxup,parameters,ownship,traffic,instantaneous_bands(parameters));
